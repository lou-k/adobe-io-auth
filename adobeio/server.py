...
def callback():
    ...
    try:
        ...
        if "output" in app.config:
            ...
            # Include the api key so a consumer application has everything it needs
            token["api_key"] = app.config["API_KEY"]
...
def profile():
    ...
    try:
        userinfo = app.config["ims_client"].userinfo(
            flask.session["credentials"]["access_token"]
        )
        return flask.render_template("index.html", response=json.dumps(userinfo))
    except HTTPError:
        logging.exception("Could not get profile")
        return flask.render_template(
            "index.html", response="profile could not be retrieved"
        )
...
def main():
    ...
    parser.add_argument(
        "-c",
        "--config",
        default="/etc/adobeio.config.json",
        help="The json file to load with your app credentials (from https://console.adobe.io/). Default: /etc/adobeio.config.json.",
    )
    parser.add_argument(
        "--cert", default="cert.pem", help="The path to the cert file for ssl."
    )
    parser.add_argument(
        "--key", default="key.pem", help="The path to the key file for ssl."
    )
    parser.add_argument(
        "--secret",
        default=None,
    ...
    parser.add_argument(
        ...
        "--scopes",
        type=str,
        default="openid,creative_sdk",
        help="The scopes to request for the app.",
    )
    parser.add_argument(
        "--host", type=str, default="localhost", help="The host to start the server on."
    )
    parser.add_argument(
        "-p", "--port", type=str, default="443", help="The port to start the server on."
    )
    parser.add_argument(
        "-d",
        "--debug",
        dest="debug",
        help="Sets the flask app to debug mode",
        action="store_true",
    )
    parser.add_argument(
        "-o",
    ...
    app.config["ims_client"] = IMS(app.config["API_KEY"], app.config["CLIENT_SECRET"])
    ...
    # Start the server
    app.run(args.host, args.port, debug=args.debug, ssl_context=(args.cert, args.key))
